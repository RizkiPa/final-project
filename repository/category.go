...
func (r *categoryRepository) GetCategoriesByUserId(ctx context.Context, id int) ([]entity.Category, error) {
	mel := []entity.Category{}
	if err := r.db.WithContext(ctx).Model(&entity.Category{}).Where("user_id = ?", id).Find(&mel).Error; err != nil {
		return []entity.Category{}, err
	}
	return mel, nil // TODO: replace this
}
...
func (r *categoryRepository) GetCategoryByID(ctx context.Context, id int) (entity.Category, error) {
	mel := entity.Category{}
	if err := r.db.WithContext(ctx).Model(&entity.Category{}).Where("id = ?", id).Find(&mel).Error; err != nil {
		return entity.Category{}, err
	}
	return mel, nil // TODO: replace this
}
